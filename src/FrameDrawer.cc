...
namespace StructureSLAM
{
    ...
    cv::Mat FrameDrawer::DrawFrame()
    {
        ...
        /*for(size_t i=0;i<vCurrentKeyLines.size();i++)
        {
            cv::line(im, cv::Point2f(vCurrentKeyLines[i].startPointX,vCurrentKeyLines[i].startPointY),cv::Point2f(vCurrentKeyLines[i].endPointX,vCurrentKeyLines[i].endPointY), cv::Scalar(0, 0, 255),4);

        }*/
        drawKeylines(im, vCurrentKeyLines, im, Scalar(0, 0, 255));     //自己添加的，绘制特征线
    ...
    void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText)
    {
        ...
        {
            if(!mbOnlyTracking)
                s << "Structure-SLAM| Points, Lines and Surface normals ";
            else
                s << ", lINE | ";
    ...
    void FrameDrawer::Update(Tracking *pTracker)
    {
        ...
        else if(pTracker->mLastProcessedState==Tracking::OK)
        {
            ...
            cout<<"FrameDrawer: update line "<<endl;
            for(int i=0; i<NL; i++)
            {
